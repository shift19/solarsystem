...
void init_entities(World *world) {
    ...
    scale_model(&world->mercury.model, 0.05, 0.05, 0.05);

    //Load the venus object and texture.
    load_model("objects\\geoid.obj", &world->venus.model);
    world->venus.texture = load_texture("textures\\venus.png");
    scale_model(&world->venus.model, 0.075, 0.075, 0.075);
    ...
    scale_model(&world->earth.model, 0.1, 0.1, 0.1);
    ...
    scale_model(&world->mars.model, 0.05, 0.05, 0.05);
    ...
    scale_model(&world->jupiter.model, 0.5, 0.5, 0.5);
    ...
    scale_model(&world->saturn.model, 0.5, 0.5, 0.5);

    //Load the uranus object and texture.
    load_model("objects\\geoid.obj", &world->uranus.model);
    world->uranus.texture = load_texture("textures\\uranus.png");
    scale_model(&world->uranus.model, 0.5, 0.5, 0.5);

    //Load the neptune object and texture.
    load_model("objects\\geoid.obj", &world->neptune.model);
    world->neptune.texture = load_texture("textures\\neptune.png");
    scale_model(&world->neptune.model, 0.5, 0.5, 0.5);
}
